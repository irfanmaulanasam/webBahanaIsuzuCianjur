      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-6">
          <Section title="Harga & Finansial">
            ...
            <Input label="Umur Pakai (tahun)" name="umurPakai" value={data.umurPakai} onChange={handleChange} info="Estimasi umur ekonomis kendaraan sebelum diganti." />
            <Input label="Nilai Sisa" name="nilaiSisa" value={data.nilaiSisa} onChange={handleChange} info="Nilai jual kembali kendaraan di akhir umur pakai." />
            <Input label="Bunga Kredit (%)" name="bungaKredit" value={data.bungaKredit} onChange={handleChange} info="Persentase bunga tahunan jika pembelian menggunakan kredit." />
          </Section>

          <Section title="Biaya Tetap">
            {/* Input lainnya */}
            <Input label="KIR & Pajak / Tahun" name="kirPajak" value={data.kirPajak} onChange={handleChange} info="Biaya tahunan seperti KIR, pajak, dan administrasi kendaraan." />
            <Input label="Asuransi / Tahun" name="asuransi" value={data.asuransi} onChange={handleChange} info="Premi tahunan asuransi kendaraan." />
            <Input label="Gaji Supir / Bulan" name="gajiSupir" value={data.gajiSupir} onChange={handleChange} info="Total gaji bulanan untuk pengemudi." />
          </Section>
        </div>
        ...
        <div className="space-y-6">
          <Section title="Biaya Variabel">
            {/* Input lainnya */}
            <Input label="Harga Solar per Liter" name="hargaSolar" value={data.hargaSolar} onChange={handleChange} info="Harga bahan bakar diesel yang digunakan." />
            <Input label="Rasio Konsumsi BBM (Km/L)" name="rasioBBM" value={data.rasioBBM} onChange={handleChange} info="Berapa kilometer yang bisa ditempuh tiap 1 liter solar." />
...
function Input({ label, name, value, onChange, info }) {
  return (
    ...
    //     type="number"
    //     name={name}
    //     // ✅ SOLUSI UNTUK UNCONTROLLED INPUT: Selalu sediakan string kosong
    //     // jika value adalah null/undefined, agar input tetap controlled.
    //     value={value ?? ''} 
    //     onChange={onChange}
    //     className="w-full border rounded-lg p-2 mt-1 text-sm"
    //   />
...
//     // PENTING: Cek apakah slug yang sudah benar (actualSlug) berbeda
//     // dari slug yang ada di URL (slug) sebelum melakukan navigate
//     if (actualSlug !== slug) {
//         // PERBAIKAN: Gunakan rute relatif /vehicle-cost/
//         navigate(`/vehicle-cost/${actualSlug}`, { replace: true }); 
//         return; // Hentikan eksekusi selanjutnya setelah redirect
//     }
    
//     // Jika actualSlug sudah sama dengan slug, lanjut proses data
//     if (found) {
//       setData(found);
//       const rawPrice = found.price.replace(/[^0-9]/g, ''); // Menghapus karakter non-angka
//       const priceNumber = parseInt(rawPrice, 10); 
...
//       }
//     } else {
//       // Fallback (hanya jika slug benar-benar tidak ditemukan)
//       const fallbackSlug = allSpecSlugs.find(s => s.includes(slug)) || allSpecSlugs[0];
      
//       // PERBAIKAN: Redirect ke slug default jika tidak ditemukan
